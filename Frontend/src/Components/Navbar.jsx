            ...
            <div className='flex items-center gap-4'>
                {token && userData ? (
                    <div className='flex items-center gap-2 cursor-pointer relative'>
                        <img onClick={() => setShowDropdown(true)} className='w-8 rounded-full' src={userData.image} alt="profile" />
                        ...
                        {showDropdown && (
                            <div className='absolute top-0 right-0 pt-14 text-base font-medium text-gray-600 z-20'>
                                <div className='min-w-48 bg-stone-100 rounded flex flex-col gap-4 p-4'>
                                    ...
                                    <NavLink onClick={()=>setShowDropdown(false)} to='/my-profile' className='hover:text-black cursor-pointer'>My Profile</NavLink>
                                    <NavLink onClick={()=>setShowDropdown(false)} to='/my-appointments' className='hover:text-black cursor-pointer'>My Appointments</NavLink>
                                    <NavLink to='/' onClick={handleLogoutClick} className='hover:text-black cursor-pointer'>Logout</NavLink>
                                </div>
                            </div>
                        )}
                    </div>
