...
const Appointment = () => {
    ...
    const {doctors, currencySymbol, backendUrl3, token, getDoctorsData, userId} = useContext(AppContext);
    ...
    const fetchDocInfo = async () => {
        try {
            const docInfo = doctors.find((item) => item.email === docId);
            setDocInfo(docInfo);
        } catch (err) {
    ...
    const BookAppointment = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${backendUrl3}/appointment/bookAppointment`, {
                userId,
                docId,
                slotDate,
                slotTime
    ...
    return (
        <div>
            <div className='flex flex-col sm:flex-row gap-4'>
                <div>
                    <img className='bg-primary w-full sm:max-w-72 rounded-lg' src={docInfo.docImg} alt={docInfo.name}/>
                </div>
                <div
