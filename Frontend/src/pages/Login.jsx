...
const Login = () => {
    ...
    const {backendUrl1, token, setToken} = useContext(AppContext)
    ...
    const onSubmitHandler = async (event) => {
        ...
        try {
            if (state === 'Sign Up') {
                response = await axios.post(`${backendUrl1}/user/register`, {email, password, name}, {
                    headers: {
                ...
                response = await axios.post(`${backendUrl1}/user/login`, userData, {
                    headers: {
                        'Content-Type': 'application/json',
        ...
        <form className='min-h-[80vh] flex items-center' onSubmit={onSubmitHandler}>
            <div
                ...
                <div className='w-full '>
                    <p>Email</p>
                    <input className='border border-[#DADADA] rounded w-full p-2 mt-1' type="email" value={email}
